...
router = APIRouter(
    prefix="/posts",
    tags=['Posts'] # for the generated documetation
)

#@router.get("/", response_model=List[schemas.PostOut]) # ezt valamiert nem szereti
@router.get("/")
...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    new_post = models.Post(owner_id=current_user.id, **post.dict()) # extract to key=value pairs as above
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    return Response(status_code=status.HTTP_204_NO_CONTENT)  #{'message': 'post was successfully deleted'}
